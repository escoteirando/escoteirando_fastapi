...
class UserPasswordService:
    ...
    def send_password_reset_email(self, username_email: str):
        ...
        if not user:
            logger.warning(
                "PASSWORD RESET REQUEST FOR NOT FOUND USER: %s", username_email
            )
            return

        auth = str(uuid1()).replace("-", "")

        if not self._cache.set_value(
            "pwd", auth, user.email, self._validade_chave.total_seconds
        ):
            logger.warning("CANNOT SET PASSWORD KEY FOR USER: %s", user.email)
            return
...
""".format(
    ...
    def redefine_password(
        ...
        logger.info("PASSWORD UPDATED FOR USER %s", password_redefine.email)
