...
def import_controllers():
    controllers_path = os.path.join(os.path.dirname(__file__), "*_controller.py")
    loading_modules = ["src.middleware.authorization_middleware"]
    ...
    for module in loading_modules:
        try:
            ...
            logger.error("ERROR WHEN LOADING CONTROLLER %s: %s", module, str(exc))
            return False
